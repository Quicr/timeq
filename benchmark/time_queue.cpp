...
static void BM_TimeQueue_Construct(benchmark::State& state)
{
    for (auto _ : state)
    {
...
static void BM_TimeQueue_Push(benchmark::State& state)
{
...
static void BM_TimeQueue_Pop(benchmark::State& state)
{
    timeq::time_queue<int> tq(state.range(0), 1, service, kIterations);
...
static void BM_TimeQueue_PopFront(benchmark::State& state)
{
    timeq::time_queue<int> tq(state.range(0), 1, service, kIterations);
    for (size_t i = 0; i < kIterations; ++i)
    {
...
static void BM_TimeQueue_PushAndPopLoaded(benchmark::State& state)
{
    timeq::time_queue<int> tq(state.range(0), 1, service, kIterations);
    int64_t items_count = 0;
...
static void BM_TimeQueue_PushAndPop_Interval_1ms(benchmark::State& state)
{
    timeq::time_queue<int> tq(state.range(0), 1, service, kIterations);
    int64_t items_count = 0;
...
static void BM_TimeQueue_PushAndPop_Interval_125ms(benchmark::State& state)
{
