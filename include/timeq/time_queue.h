...
class time_queue
{
  protected:
    ...
    struct queue_value_type
    {
        queue_value_type(bucket_type& bucket, index_type value_index, tick_type expiry_tick, tick_type wait_for_tick)
            : bucket(std::addressof(bucket)), value_index(value_index), expiry_tick(expiry_tick),
  ...
  public:
     ...
     *
     * @throws std::invalid_argument If the duration or interval do not meet requirements or the tick_service is null.
     */
    time_queue(std::size_t duration, std::size_t interval, std::shared_ptr<tick_service> tick_service,
               std::size_t initial_queue_size)
        : _duration{duration}, _interval{interval}, _tick_service(std::move(tick_service))
    {
