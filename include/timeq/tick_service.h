...
class threaded_tick_service : public tick_service
{
  ...
  public:
    threaded_tick_service(std::uint64_t sleep_delay_us = 333) : _sleep_delay_us{sleep_delay_us}
    {
        _tick_thread = std::thread(&threaded_tick_service::tick_loop, this);
    }

    threaded_tick_service(const threaded_tick_service& other)
        : _ticks{other._ticks.load(std::memory_order_relaxed)}, _sleep_delay_us{other._sleep_delay_us},
  ...
  private:
    void tick_loop()
    {
        ...
        while (!_stop)
        {
            ...
            const std::uint64_t delta = std::chrono::duration_cast<std::chrono::microseconds>(now - prev_time).count();
